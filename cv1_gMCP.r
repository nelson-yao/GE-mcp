...
lammax <- function(x,y,group)
{
  ...
  for ( j in 1:p )
  {
    ...
    else if ( end[j]-start[j]==1)
      inner[j]=sum(sum(x[,start[j]:end[j]]*as.vector(y)/n)^2)^0.5/group[j]^0.5
  }
...
sse.evaluate <- function(i,X,Y,W,fold.d,group,lambda,gamma)
{
  ...
  fit <- gmcp(Y.train,X.train,W.train,lambda,gamma)
...
parCv <- function(cl,lambda.seq,X,Y,W,group,fold.d,n.fold,gamma)
{
  ...
  {
    lambda=lambda.seq[i]
    #v <- lapply(index,fun,X,Y,W,fold.d,n.fold,group,lambda,gamma)
...
cv.optim <- function(X,Y,W,group,n.fold,epsilon,alpha,n.step,gamma,cl)
{
  ...
  l.max <- lammax(W.t,r.alpha,group)$lambda.max
