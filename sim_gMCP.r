...
sim <- function(num,p,q,mag1,mag2,mag,gamma, epsilon, replic)
{
  ...
  for ( i in 1:replic)
  {
    ...
    outfile<- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/gene_s_match_surv_gMCP_",num,"_",p,"_",mag1,"_",mag2,"_",gamma,".txt"))
  ...
  outfile1 <- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/sse_surv_gMCP_",num,"_",p,"_",mag1,"_",mag2,"_",gamma,".txt"))
...
sim.band <- function(num,p,q,mag2,mag,gamma, epsilon, replic)
{
  ...
  for ( i in 1:replic)
  {
    ...
    index=seq(1,ncol(W)); 
    s.gene=index[fit$beta!=0]
    outfile<- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/gene_s_match_surv_gMCP_",num,"_",p,"_",mag2,"_",gamma,".txt"))
    write.table(matrix(s.gene,nrow=1),outfile,sep=" ",quote=F, row.names=F,col.names=F,append=T);
  ...
  outfile1 <- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/sse_surv_gMCP_",num,"_",p,"_",mag2,"_",gamma,".txt"))
  write.table(sse.seq,outfile1,sep=",",quote=F, row.names=F,col.names=F,append=T);
...
sim.band1 <- function(num,p,q,mag2,mag,gamma, epsilon, replic)
{
  ...
  for ( i in 1:replic)
  {
    ...
    index=seq(1,ncol(W)); 
    s.gene=index[fit$beta!=0]
    outfile<- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/gene_s_match_surv_b1_gMCP_",num,"_",p,"_",mag2,"_",gamma,".txt"))
    write.table(matrix(s.gene,nrow=1),outfile,sep=" ",quote=F, row.names=F,col.names=F,append=T);
  ...
  outfile1 <- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/sse_surv_b1_gMCP_",num,"_",p,"_",mag2,"_",gamma,".txt"))
  write.table(sse.seq,outfile1,sep=",",quote=F, row.names=F,col.names=F,append=T);
...
#source("/space/jliu20/GE_interaction/Group_MCP/sim_gMCP.r")
#source("/space/jliu20/GE_interaction/generate.surv.r")
#s = sim(150,1000,3,0.2,0.05,1,6,0.05,5); #node
#s = sim(150,1000,3,0.5,0.05,1,6,0.02,5); #node
#s = sim(150,1000,3,0.8,0.05,1,6,0.02,5); #node

#s = sim(250,1000,3,0.2,0.05,1,6,0.05,5); #node
#s = sim(250,1000,3,0.5,0.05,1,6,0.02,5); #node
#s = sim(250,1000,3,0.8,0.05,1,6,0.02,5); #node

#source("/space/jliu20/GE_interaction/Group_MCP/sim_gMCP.r")
#source("/space/jliu20/GE_interaction/generate.surv.band.r")
#s = sim.band(150,1000,3,0.05,1,6,0.02,5);   #node
#s = sim.band(250,1000,3,0.05,1,6,0.01,5);   #node

#source("/space/jliu20/GE_interaction/Group_MCP/sim_gMCP.r")
#source("/space/jliu20/GE_interaction/generate.surv.band1.r")
#s = sim.band1(150,1000,3,0.05,1,6,0.05,5);  #node
