sim <- function(num,p,q,mag1,mag2,mag,replic)
{
  ...
  for ( i in 1:replic)
  {
    ...
    outfile<- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/Categorical/gene_s_match_surv_Marg_",num,"_",p,"_",mag1,"_",mag2,".txt"))
...
sim.band <- function(num,p,q,mag2,mag,replic)
{
  ...
  for ( i in 1:replic)
  {
    ...
    pvalue <- as.vector(pvalue)
    s.gene <- index[pvalue < 5e-05]
    outfile<- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/Categorical/gene_s_match_surv_Marg_",num,"_",p,"_",mag2,".txt"))
    write.table(matrix(s.gene,nrow=1),outfile,sep=" ",quote=F, row.names=F,col.names=F,append=T);
...
sim.band1 <- function(num,p,q,mag2,mag,replic)
{
  ...
  for ( i in 1:replic)
  {
    ...
    pvalue <- as.vector(pvalue)
    s.gene <- index[pvalue < 5e-05]
    outfile<- gsub("( )", "", paste("/space/jliu20/GE_interaction/Sim/Categorical/gene_s_match_surv_b1_Marg_",num,"_",p,"_",mag2,".txt"))
    write.table(matrix(s.gene,nrow=1),outfile,sep=" ",quote=F, row.names=F,col.names=F,append=T);
...
#source("/space/jliu20/GE_interaction/sim.trad.categorical.r")
#source("/space/jliu20/GE_interaction/generate.surv.categorical.r")
#s = sim(150,1000,3,0.2,0.05,1,5); #node04
#s = sim(150,1000,3,0.5,0.05,1,5); #node04
#s = sim(150,1000,3,0.8,0.05,1,5); #node04

#s = sim(250,1000,3,0.2,0.05,1,5); #node06
#s = sim(250,1000,3,0.5,0.05,1,5); #node06
#s = sim(250,1000,3,0.8,0.05,1,5); #node06


#source("/space/jliu20/GE_interaction/sim.trad.categorical.r")
#source("/space/jliu20/GE_interaction/generate.surv.band.categorical.r")
#s= sim.band(150,1000,3,0.05,1,5);  #node07
#s= sim.band(250,1000,3,0.05,1,5);  #node07

#source("/space/jliu20/GE_interaction/sim.trad.categorical.r")
#source("/space/jliu20/GE_interaction/generate.surv.band1.categorical.r")
#s= sim.band1(150,1000,3,0.05,1,5);   #node07
#s= sim.band1(250,1000,3,0.05,1,5);   #node07
